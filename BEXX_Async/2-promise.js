...
const networkReq = new Promise((resolve, reject) => {  // doğru çalışırsa 1. parametre, yanlış çalışırsa 2. parametre çalışır. Parametre isimlerinin hiçbir önemi yok. 
  const data = { a: 1, b: 2 };
  const success = Math.floor(Math.random() * 2); //?(0,1)
  if (success) {  // eğer 1 gelirse success true olacak ve if çalışacak
    console.log("Data fetched");
    resolve(data);
    // console.log(data);
  } else {// eğer 0 gelirse success false olacak ve if çalışacak
...
networkReq
  ...
  .catch((error) => document.write(error));  // Catch ile error handler yaparız yani hatayı ele alırız, hatayı yakalarız.
